...
@implementation CupsViewController
@synthesize tnamount,mywebview,strMertId,strpaymtkey;
...
//    CustomerModule *daocustomer=[[CustomerModule alloc]init];
//    TblCustomer *tblcus=[daocustomer getCustmerList];
//    // Txn cart detail
//    CartModule *daocart=[[CartModule alloc]init];
//    NSMutableArray *txncart = [daocart getCartList:[tblcus customer_id]];
//    NSMutableArray *dicpayment = [self buildtxncart:txncart];
//    
//    
//    NSDictionary *dict = @{@"transactionId":@"0", @"transactionNo":txnno,@"transactionTotal":tnamount, @"customerId":[tblcus customer_id], @"placeId":[tblcus place_id],@"transactionDate":[Utils getCurrentDate:@"dd-MM-yyyy"], @"hashCode":@"0", @"payReceipt":payreceipt, @"createdOn":[Utils getCurrentDate:@"dd-MM-yyyy HH:mm:ss"],@"status":@"A",@"transdtls" :dicpayment,@"payResponse":payrespnse,@"couponType":@"C"};
//    
//    NSLog(@"%@",dict);
//    
//    NSString* pstrurl=[NSString stringWithFormat:@"%@%@",[Utils pathsetting],[NSString stringWithFormat:@"ws/v6/transaction"]];
//    
//    NSError *error;
...
- (void)fetchedDetl:(NSData *)requestHandler {
    ...
    NSDictionary* arritems = [resdict valueForKey:@"response"];
    NSString* response_code = [arritems valueForKey:@"response_code"];
