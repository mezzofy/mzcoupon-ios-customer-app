#import "MZJastor.h"
#import "MZJastorRuntimeHelper.h"

@implementation MZJastor
...
- (id)initWithDictionary:(NSDictionary *)dictionary {
	...
	if ((self = [super init])) {
		for (NSString *key in [MZJastorRuntimeHelper propertyNames:[self class]]) {
            ...
            if ([MZJastorRuntimeHelper isPropertyReadOnly:[self class] propertyName:key]) {
                continue;
            }
			
			// handle dictionary
			if ([value isKindOfClass:nsDictionaryClass]) {
				Class klass = [MZJastorRuntimeHelper propertyClassForPropertyName:key ofClass:[self class]];
			...
			// handle array
				...
				for (id child in value) {
                    if ([[child class] isSubclassOfClass:nsDictionaryClass]) {
                        ...
                        } else if ([arrayItemType isSubclassOfClass:[MZJastor class]]) {
                            MZJastor *childDTO = [[[arrayItemType alloc] initWithDictionary:child] autorelease];
...
- (NSMutableDictionary *)toDictionary {
	...
	for (NSString *key in [MZJastorRuntimeHelper propertyNames:[self class]]) {
        ...
        if (value && [value isKindOfClass:[MZJastor class]]) {
            ...
            if (internalValue && [internalValue isKindOfClass:[MZJastor class]]) {
...
- (NSDictionary *)map {
	NSArray *properties = [MZJastorRuntimeHelper propertyNames:[self class]];
...
- (BOOL)isEqual:(id)object {
	if (object == nil || ![object isKindOfClass:[MZJastor class]]) return NO;
	
	MZJastor *model = (MZJastor *)object;
