...
+ (NSData *)urlGetRequest:(NSString *)purl param:(NSMutableDictionary*)pparam {
    NSURL *baseURL = [NSURL URLWithString:[Utils pathsetting]];
    NSLog(@"%@", baseURL);
    
    __block NSData *retdata = NULL;
    
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
    NSData *plainData = [[NSString stringWithFormat:@"%@:%@", BASIC_KEY,BASIC_SECRET] dataUsingEncoding:NSUTF8StringEncoding];
...
+ (NSData *)urlRegGetRequest:(NSString *)purl param:(NSMutableDictionary*)pparam {
    NSURL *baseURL = [NSURL URLWithString:[Utils pathsettingforreg]];
    NSLog(@"%@", baseURL);
